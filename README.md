...
| [`.ai/scripts/lab-status.ps1`](.ai/scripts/lab-status.ps1) | The session-readiness check described under "Start here". It is hand-maintained rather than one of the generated launchers the row above describes, so `om-setup-agent-pipeline` neither creates nor regenerates it. |
| [`.ai/scripts/lab-report.ps1`](.ai/scripts/lab-report.ps1) | Renders the lab's current state — repository status, installed skills, available specs, and the learning log — as one Markdown document, for pasting into a PR, an issue, or a note. Read-only; `-OutFile` writes the file (do not use `>`, which writes UTF-16 in PowerShell 5.1). Hand-maintained, like the row above. It describes rather than gates: for a readiness verdict, run `lab-status.ps1`. |
