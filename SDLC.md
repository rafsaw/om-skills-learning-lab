...
**The QA gate is currently off in this repository** (`qaGate: false` in `.ai/agentic.config.json`). `needs-qa` is therefore advisory: it flags a PR as worth manual exercise, but it does not hard-block the merge the way it would with the gate on. `qa-failed`, `blocked`, and `do-not-merge` remain hard blocks. Turn the gate on by setting `qaGate: true` and re-running `om-setup-agent-pipeline` when this repo starts shipping a user-facing surface.
...
This repository currently defines **no CI workflows** (`.github/workflows/` does not exist), so in practice there is nothing to wait for and no required checks to gate a merge — the local validation gate below is the only automated evidence. Add workflows and this section starts applying as written, with no config change needed.
...
This repository has no build system — no `package.json`, `Makefile`, or language manifest — because its content is Markdown skill definitions rather than compiled code. The gate is therefore a hygiene check: `git diff --check` exits non-zero on trailing whitespace and leftover conflict markers, which are the realistic mechanical defects in a Markdown-only change.

Any non-zero exit fails the gate and blocks the PR. The implementing skills run the gate before opening a PR, and `om-check-and-commit` runs it before pushing a hand-worked branch. The command list lives in `.ai/agentic.config.json`; when it changes, update it there and in this section together. When this repo grows a real toolchain (a linter, a link checker, a test runner for skill validation), add those commands to the config and re-run `om-setup-agent-pipeline`.
